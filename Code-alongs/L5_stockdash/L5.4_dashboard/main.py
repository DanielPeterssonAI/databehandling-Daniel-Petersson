...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col(html.P("Choose a stock"), className = "mt-1", 
            xl = {"size": 2, "offset": 2}, lg = 4,
        ),
        dbc.Col(
            dcc.Dropdown(id = 'stock-picker-dropdown', className = 'm-1',
    ...
    # stores an indermediate value on clients brwser for sharing between callbacks
...
def filter_df(stock, time_index):
    """Filters dataframe and stores in intermediary for callbacks 
    Returns:
...
def update_graph(json_df, stock, ohlc):
    ...
    return fig # fig object goes into output, i.e. figure property
